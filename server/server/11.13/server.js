...
app.post('/board', async (req, res) => {
	...
	if(search==null){
		sql = 'SELECT board.*, member.address FROM board LEFT JOIN member ON board.userid = member.userid'
		const data = await conn.query(sql);
		conn.release();
		
		res.status(200).json({items:data});
	}else{
		sql = 'SELECT board.*, member.address FROM board LEFT JOIN member ON board.userid = member.userid where title like ?'
		const data = await conn.query(sql,["%"+search+"%"]);
		
		const search_data = await conn.query("select search from search where userid=? and search=?",[userid,search]);
		if(search_data.length > 0){
			
		}else{
			await conn.query("insert into search values (?,?)",[userid, search]);
		}
		conn.release();
...
app.post('/alert', async (req, res) => {
	...
	const conn = await getConnection();
	const data = await conn.query('select num,title from board where num in (select boardid from alert where userid=?)',[userid]);
	conn.release();
