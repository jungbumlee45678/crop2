...
import com.example.ut.databinding.FragmentHomeBinding
...
class HomeFragment : Fragment() {
    ...
    override fun onCreateView(
            ...
            override fun onResponse(call: Call<board>, response: Response<board>) {
                if (response.isSuccessful) {
                    ...
                    if(responseBody!!.items.isNotEmpty()){
                        val scrollView  = view.findViewById<ScrollView>(R.id.scroll) // 부모 레이아웃 ID로 변경
                        val linearLayout = scrollView.getChildAt(0) as ViewGroup

                        linearLayout.removeViewAt(0)

                        println("i:${responseBody!!.items.size}")
                        for(i in 0 until responseBody!!.items.size){
                            ...
                            linearLayout1.setOnClickListener {
                                intent(responseBody.items[i].title,
        ...
        val wirth_button = view.findViewById<Button>(R.id.write)
        wirth_button.setOnClickListener{
            val intent = Intent(activity, write_page::class.java)
            startActivity(intent)
        }
