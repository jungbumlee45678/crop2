...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(TAG == null){
            ...
            setFragment(TAG_chat, ChattingFragment())

            val chatMenuItem = binding.navigationView.menu.findItem(R.id.navigation_chat)
            chatMenuItem.isChecked = true
        } else if(TAG.equals("info")){
            setFragment(TAG_ANOTHER, InformationFragment())

            val chatMenuItem = binding.navigationView.menu.findItem(R.id.navigation_person)
            chatMenuItem.isChecked = true
        }
        ...
        binding.navigationView.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.navigation_chat -> setFragment(TAG_chat, ChattingFragment())
    ...
    private fun setFragment(tag: String, fragment: Fragment) { //프레그먼트 함수
        ...
        val notificationsFragment = manager.findFragmentByTag(TAG_chat)
        val anotherFragment = manager.findFragmentByTag(TAG_ANOTHER)

        hideFragments(fragTransaction, homeFragment, notificationsFragment, anotherFragment)

        if (tag == TAG_HOME && homeFragment != null) {
            fragTransaction.show(homeFragment) // 보여줄 화면 설정
        } else if (tag == TAG_chat && notificationsFragment != null) {
